...
const CATEGORIES = [
  ...
  { id: 'mystic',    name: 'Mystic Shop',      icon: '🔮', query: 'mystic candle crystal' },
];
...
const PRODUCT_NAMES = {
  ...
  mystic:  ['Buona Fortuna Giornaliera', 'Energia Positiva per Esame', 'Protezione Simbolica Viaggi', 'Chiarezza Mentale Oggi', 'Rituale Serenita Notturna', 'Boost Autostima 24h', 'Equilibrio Emotivo Giornaliero', 'Sblocco Opportunita'],
};
...
const DESCS = [
  ...
  "Un rituale simbolico per iniziare la giornata con intenzione e chiarezza.",
  "Energia positiva canalizzata per supportare il tuo momento.",
  "Protezione simbolica e serenita interiore, disponibile ora.",
  "Un momento di connessione con te stesso, guidato dall'intenzione.",
];



// ─── SEED OFFSET per categoria (picsum.photos/seed/{n}) ────────────────────
// Range separati → ogni categoria cade su immagini visivamente diverse
// Valori scelti dopo verifica manuale dei seed picsum
const CAT_SEED_OFFSET = {
  tech:    100,   // seed 100-149: gadget, schermi, tech
  fashion: 200,   // seed 200-249: abbigliamento, accessori
  home:    300,   // seed 300-349: interni, arredo
  beauty:  400,   // seed 400-449: cosmetici, beauty
  sport:   500,   // seed 500-549: sport, fitness
  food:    600,   // seed 600-649: cibo, cucina
  books:   700,   // seed 700-749: libri, arte
  travel:  800,   // seed 800-849: natura, outdoor
  pets:    900,   // seed 900-949: animali
  gaming:  1000,  // seed 1000-1049: gaming, tech scuro
  mystic:  1100,  // seed 1100-1149: candele, cristalli, luce soffusa
};
...
const CATEGORY_KEYWORDS = {
  ...
  mystic:  ['candle flame','crystal healing','incense smoke','soft light meditation','spiritual ritual','amethyst crystal','candle glow','sage smudge','chakra stones','zen meditation'],
};
...
function generateProducts(count = 120) {
  ...
  CATEGORIES.forEach(cat => {
    ...
    for (let i = 0; i < perCat && products.length < count; i++) {
      ...
      const isMystic = cat.id === 'mystic';
      const price    = isMystic
        ? parseFloat((Math.random() * 8 + 1.99).toFixed(2))
        : parseFloat((Math.random() * 88 + 5.99).toFixed(2));
      ...
      // Immagine coerente via loremflickr keyword + lock per stabilità
      const imgKw    = getProductKeyword(cat.id, baseName, id);
      const imgLock  = id * 13; // lock stabile per prodotto
      const imgUrl   = `https://loremflickr.com/400/400/${encodeURIComponent(imgKw)}?lock=${imgLock}`;
      const thumbUrl = `https://loremflickr.com/80/80/${encodeURIComponent(imgKw)}?lock=${imgLock}`;
  ...
  return `
    <div class="product-card" data-id="${p.id}" onclick="openProductModal(${p.id})">
      <div class="card-img-wrap">
        <img src="${p.imgUrl}" alt="${p.name}" loading="lazy" onerror="this.src='https://loremflickr.com/400/400/product'">
        ${badgeHtml}
