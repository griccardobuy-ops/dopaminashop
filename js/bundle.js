...
function generateProducts(count = 120) {
  ...
  CATEGORIES.forEach(cat => {
    ...
    for (let i = 0; i < perCat && products.length < count; i++) {
      ...
      // Immagine coerente: seed = offset categoria + product id (stabile al refresh)
      const catOffset = CAT_SEED_OFFSET[cat.id] || 0;
      const imgSeed   = catOffset + (id % 50);
      const imgUrl    = `https://picsum.photos/seed/${imgSeed}/400/400`;
      const thumbUrl  = `https://picsum.photos/seed/${imgSeed}/80/80`;
  ...
  return `
    <div class="product-card" data-id="${p.id}" onclick="openProductModal(${p.id})">
      <div class="card-img-wrap">
        <img src="${p.imgUrl}" alt="${p.name}" loading="lazy" onerror="this.src='https://picsum.photos/seed/fallback/400/400'">
        ${badgeHtml}
